...
public class Exercise1_14 {
    public static void main(String[] args) {
        ...
        int int_num = (4*arr_num[0] + 3*arr_num[1] + 2*arr_num[2] + 1*arr_num[3] + 0*arr_num[4]);
        System.out.format("CAL = 4 x %d + 3 x %d + 2 x %d + 1 x %d + 0 x %d%n", arr_num[0], arr_num[1], arr_num[2], arr_num[3], arr_num[4]);
        System.out.format("CAL : %d%n", int_num);
        System.out.format("AVG : %d / %d%n", int_num, Arrays.stream(arr_num).sum());
        System.out.format("AVG : %.4f%n", (((float)int_num) / Arrays.stream(arr_num).sum()));
    }
}